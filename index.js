...
async function startElainaBot() {
	const { state, saveCreds } = await useMultiFileAuthState('Restun');
	...
	const getMessage = async (key) => {
		...
		return {
			conversation: 'Halo Saya Elaina Bot'
		}
	}
	
	const Elaina = WAConnection({
		logger: level,
		getMessage,
	...
	if (pairingCode && !phoneNumber && !Elaina.authState.creds.registered) {
		...
		(async () => {
			await getPhoneNumber();
			await exec('rm -rf ./Restun/*');
			console.log('Phone number captured. Waiting for Connection...\n' + chalk.blueBright('Estimated time: around 2 ~ 5 minutes'))
		})()
	}
	
	await Solving(Elaina, store)
	
	Elaina.ev.on('creds.update', saveCreds)
	
	Elaina.ev.on('connection.update', async (update) => {
		const { qr, connection, lastDisconnect, isNewLogin, receivedPendingNotifications } = update
		if (!Elaina.authState.creds.registered) console.log('Connection: ', connection || false);
		if ((connection === 'connecting' || !!qr) && pairingCode && phoneNumber && !Elaina.authState.creds.registered && !pairingStarted) {
			setTimeout(async () => {
				pairingStarted = true;
				console.log('Requesting Pairing Code...')
				let code = await Elaina.requestPairingCode(phoneNumber);
		...
		if (connection === 'close') {
			...
			if (reason === DisconnectReason.connectionLost) {
				...
				Elaina.end(`Unknown DisconnectReason : ${reason}|${connection}`)
			}
		}
		if (connection == 'open') {
			console.log('Connected to : ' + JSON.stringify(Elaina.user, null, 2));
			let botNumber = await Elaina.decodeJid(Elaina.user.id);
			if (global.db?.set[botNumber] && !global.db?.set[botNumber]?.join) {
				if (my.ch.length > 0 && my.ch.includes('@newsletter')) {
					if (my.ch) await Elaina.newsletterMsg(my.ch, { type: 'follow' }).catch(e => {})
	...
	Elaina.ev.on('contacts.update', (update) => {
		for (let contact of update) {
			...
			if (contact.id.endsWith('@lid')) {
				trueJid = Elaina.findJidByLid(contact.id, store);
			} else {
	...
	Elaina.ev.on('call', async (call) => {
		let botNumber = await Elaina.decodeJid(Elaina.user.id);
		if (global.db?.set[botNumber]?.anticall) {
			for (let id of call) {
				if (id.status === 'offer') {
					let msg = await Elaina.sendMessage(id.from, { text: `Saat Ini, Kami Tidak Dapat Menerima Panggilan ${id.isVideo ? 'Video' : 'Suara'}.\nJika @${id.from.split('@')[0]} Memerlukan Bantuan, Silakan Hubungi Owner :)`, mentions: [id.from]});
					await Elaina.sendContact(id.from, global.owner, msg);
					await Elaina.rejectCall(id.id, id.from)
				}
			}
		}
	});
	
	Elaina.ev.on('messages.upsert', async (message) => {
		await MessagesUpsert(Elaina, message, store);
	});
	
	Elaina.ev.on('group-participants.update', async (update) => {
		await GroupParticipantsUpdate(Elaina, update, store);
	});
	
	Elaina.ev.on('groups.update', (update) => {
	...
	Elaina.ev.on('presence.update', ({ id, presences: update }) => {
		store.presences[id] = store.presences?.[id] || {};
		Object.assign(store.presences[id], update);
	});
	
	setInterval(async () => {
		if (Elaina?.user?.id) await Elaina.sendPresenceUpdate('available', Elaina.decodeJid(Elaina.user.id)).catch(e => {})
	}, 10 * 60 * 1000);
